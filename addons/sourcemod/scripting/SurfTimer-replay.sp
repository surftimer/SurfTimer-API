...
ConVar g_hWrcpBot = null;										// Stages bot?
...
ConVar g_hReplayBotPlayerModel = null;							// Replay bot model
...
{
	name = "SurfTimer-Replays",
	author = "All contributors",
	description = "Record and replay SurfTimer runs",
	version = VERSION,
	url = "https://github.com/surftimer/Surftimer-Official"
};

// Initiate all needed variables for the Replays plugin to work separately from SurfTimer
public void OnPluginStart()
{
	char g_szRecordHolder[MAX_NAME_LENGTH], g_szRecordTime[32];

	AutoExecConfig_SetCreateDirectory(false);
	AutoExecConfig_SetCreateFile(false);
	AutoExecConfig_SetFile("surftimer");

	// Get ConVar values from surftimer.cfg
	...
	// SurfTimer include variables
	surftimer_GetMapData(g_szRecordHolder, g_szRecordTime, g_fRecordMapTime);
	g_TotalStages = surftimer_GetMapStages();
}
